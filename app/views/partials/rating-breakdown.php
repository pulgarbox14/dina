 ...
 * @var string|null $filterUrl  URL de la page produit : chaque ligne devient un filtre (?note=N#avis)
    ...
    <?php foreach ($stats['distribution'] as $n => $total):
        ...
        <li>
            <?php if ($filterUrl !== null && $total > 0): ?>
                <a href="<?= e($filterUrl . '?note=' . $n . '#avis') ?>" class="flex items-center gap-3 text-sm rounded-lg -mx-2 px-2 py-0.5 hover:bg-[var(--bg-elevated)] <?= $active === $n ? 'bg-[var(--bg-elevated)] font-semibold' : '' ?>"
